...
public abstract class Projectile implements Entity{ //Classe projectile abstraite (sert de base)
	...
	//Constructeur par défaut + objets projectiles référenciés
	...
	private void calculateDirection() {
		float totalAllowedMovement = 1.0f;//Max référence
		...
		float totalDistanceFromTarget = xDistanceFromTarget + yDistanceFromTarget; //Calculer la diastance totale jusqu'à la cible
		float xPercentOfMovement = xDistanceFromTarget / totalDistanceFromTarget;
		xVelocity = xPercentOfMovement;
		yVelocity = totalAllowedMovement - xPercentOfMovement;
		//Placer la direction en fonction de la position de l'ennemi relative à la tourelle
		if (target.getX() < x)
			xVelocity *= -1;//Inverser la vélocité de X
		if (target.getY() < y)
			yVelocity *= -1;//Inverser la vélocité de Y
	}
	
	//Faire des dégats aux ennemis
	public void damage() {
